...
// api_key:
// auth_token:
// tnid:     The ID of the tenant to add the products to.
//
...
function ciniki_artcatalog_productAdd(&$ciniki) {
    ...
    if( $args['artcatalog_id'] == '0' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.artcatalog.30', 'msg'=>'No artcatalog item specified'));
    }
    ...
    $rc = ciniki_artcatalog_checkAccess($ciniki, $args['tnid'], 'ciniki.artcatalog.productAdd'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }   
    ...
    if( $rc['num_rows'] > 0 ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.artcatalog.31', 'msg'=>'You already have a product with this name, please choose another name'));
    }
    ...
    return ciniki_core_objectAdd($ciniki, $args['tnid'], 'ciniki.artcatalog.product', $args);
}
?>
