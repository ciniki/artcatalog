...
function ciniki_artcatalog_settings() {
    ...
    this.main.sections = {
        ...
        '_buttons':{'label':'', 'buttons':{
            'save':{'label':'Save', 'fn':'M.ciniki_artcatalog_settings.main.save();'},
            }},
    }
    ...
    this.main.open = function(cb) {
        M.api.getJSONCb('ciniki.artcatalog.settingsGet', {'tnid':M.curTenantID}, function(rsp) {
            if( rsp.stat != 'ok' ) {
                M.api.err(rsp);
                return false;
            }
    ...
    this.main.save = function() {
        ...
        if( c != '' ) {
            M.api.postJSONCb('ciniki.artcatalog.settingsUpdate', {'tnid':M.curTenantID}, c, 
                function(rsp) {
                    if( rsp.stat != 'ok' ) {
                        M.api.err(rsp);
                        return false;
    ...
    this.main.addButton('save', 'Save', 'M.ciniki_artcatalog_settings.main.save();');
